...
| PyPI distribution | `get-done` | `python -m pip install get-done` |
...
The `getdone` distribution name is already occupied on PyPI. Python distribution names
share a global namespace, while an installed command and import package may use a different
name. Therefore this project publishes as `get-done` while retaining GetDone as the
product name and `getdone` as the command and import namespace.

Do not attempt to publish this project under the existing `getdone` PyPI name, depend on
ownership transfer, or use confusing punctuation variants. Treat `get-done` as the
...
but the command must say `pip install get-done`.
