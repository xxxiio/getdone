...
Codex uses the bootstrap-managed `AGENTS.md`. ChatGPT uses conversational or workspace references and requires no additional project file. See [Agent adapter contracts](adapters.md).
...
See [Release process](releasing.md) for the signing and verification policy.
